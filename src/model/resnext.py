...
class ResNeXtBottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, cardinality, base_width, widen_factor):
        ...
        self.equalChannels = in_channels == out_channels
        print("are equals:", self.equalChannels)
...
#        if self.equalChannels and self.training:
#            rrr = torch.rand(1).item()
#            if rrr > 0.5:
#                return x
...
class ResNext(nn.Module):
    def __init__(self, config):
        ...
        cardinality = config.model.cardinality
...
if __name__ == '__main__':
    net = CifarResNeXt(8, 29, 340, 64, 4).cuda()
    i = Variable(torch.randn(6, 3, 64, 64)).cuda()
    y = net(i)
    print(y)
    print(y.shape)
